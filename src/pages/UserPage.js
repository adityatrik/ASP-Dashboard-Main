...
export default function UserPage() {
  ...
  const handleClear = (id,area) => {
    console.log(id);
    console.log(area);
    setIsLoading(true);
    axios.post('http://localhost:3003/api/clear', {
    })
      .then(response => {
  ...
  return (
    <>
      <Helmet>
        <title>ALERT SYSTEM | Perangkat</title>
      </Helmet>
                  ...
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    ...
                    return (
                      <TableRow hover key={id}>
                        ...
                        <TableCell align="left">
                          {/* <Button size="small" variant="outlined">CLEAR ALARM</Button> */}
                          <Button size="small" variant="outlined" onClick={() => handleClear(ipDevice,area)} disabled={isLoading}> {isLoading ? <CircularProgress size={24} color="primary" /> : 'CLEAR ALARM'}</Button>
                        </TableCell>
                      </TableRow>
                    );
                  })}
